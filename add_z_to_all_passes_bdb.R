...
passing_plays <- read_csv("/Users/Rishav/Documents/nflScrapR/parabolizR/data/bdb_passing_play_index_expanded_full.csv")
passing_plays <- passing_plays %>% subset(PassResult != "S")
passing_plays$v <- 0
passing_plays$vxy <- 0
passing_plays$vz <- 0
...
doWork <- function(passing_plays,p, i) {
  play <- p
  play_file <- paste(
    "/Users/Rishav/Documents/Big-Data-Bowl/Data/tracking_gameId_",
    play$gameId, ".csv",
    sep=""
  )
  play_file_name <- paste(
    "/Users/Rishav/Documents/nflScrapR/parabolizR/data/bdb/",
    "2017", "_",
    play$possessionTeam, "_",
    play$gameId, "_",
    play$playId, "_withZ.csv",
    sep=""
  )
  
  a <- read_csv(play_file)
  a <- a %>% subset(playId == play$playId)
...
write.csv(passing_plays, "/Users/Rishav/Documents/nflScrapR/parabolizR/data/bdb_passing_play_index_expanded_full.csv")
