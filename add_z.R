...
add_z_to_play_with_velocity_big_data <- function(a){
  ...
  pass_forward_frame = min(allplayers_passforward$frameId)
  allplayers_passarrived <- a %>% subset(event == "pass_arrived" | event == "pass_outcome_interception" | event == "pass_outcome_touchdown" | event == "pass_outcome_caught" | event == "pass_outcome_incomplete")
  pass_arrived_frame = min(allplayers_passarrived$frameId)
  only_players <- a %>% subset(displayName != 'Football')
  only_players$seconds_since_throw = (only_players$frameId - pass_forward_frame)/10
  only_players$z = 0
  only_players$z1 = 0
  only_players$z2 = 0
  ...
  specific <- just_football
  specific$teamAbbr <- "ball"
  specific$seconds_since_throw = (specific$frameId- pass_forward_frame)/10
  start_x <- specific %>% subset(frameId == pass_forward_frame) %>% pull(x)
  start_y <- specific %>% subset(frameId == pass_forward_frame)  %>% pull(y)
  end_x <- specific %>% subset(frameId == pass_arrived_frame) %>% pull(x)
  end_y <- specific %>% subset(frameId == pass_arrived_frame)  %>% pull(y)
  d <- sqrt((start_x - end_x)*(start_x - end_x) + (start_y - end_y)*(start_y - end_y)) #yards
  vxy = d / ((pass_arrived_frame - pass_forward_frame)/10) # 10 frames a second. vxy in yards per second
  bigT = specific %>% subset(frameId == pass_arrived_frame)  %>% pull(seconds_since_throw)
  vz = (bigT*g)/2
  vz1 = (0.5+0.5*g*bigT*bigT)/bigT
  vz2 = (-0.5+0.5*g*bigT*bigT)/bigT
  ...
  specific$z1 = 1.5 + vz1*specific$seconds_since_throw - 0.5*g*(specific$seconds_since_throw^2)
  specific$z2 = 2.5 + vz2*specific$seconds_since_throw - 0.5*g*(specific$seconds_since_throw^2)
  specific$z[specific$frameId > pass_arrived_frame] = 0
  specific$z[specific$frameId < pass_forward_frame] = 0
  specific$z1[specific$frameId > pass_arrived_frame] = 0
  specific$z1[specific$frameId < pass_forward_frame] = 0
  specific$z2[specific$frameId > pass_arrived_frame] = 0
  specific$z2[specific$frameId < pass_forward_frame] = 0
  totaloutput <- rbind(only_players,specific)
